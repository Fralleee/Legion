...
Shader "Custom/AddMul_SS_Cull_Depth" {
    Properties {
        ...
        [MaterialToggle] _HasFinalSS_Add ("HasFinalSS_Add", Float ) = 0
        _FinalSS_Add ("FinalSS_Add", 2D) = "black" {}
        _FinalSS_Add_Power ("FinalSS_Add_Power", Float ) = 1
        [MaterialToggle] _HasAdditiveFresnel ("HasAdditiveFresnel", Float ) = 0
        _AddFresnelPower ("AddFresnelPower", Float ) = 1.5
        _AddFresnel_Color ("AddFresnel_Color", Color) = (1,1,1,1)
        _AddFresnel_Intensity ("AddFresnel_Intensity", Float ) = 2
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _FinalSS_Add; uniform float4 _FinalSS_Add_ST;
            uniform fixed _HasFinalSS_Add;
            uniform float _FinalSS_Add_Power;
            uniform fixed _HasAdditiveFresnel;
            uniform float _AddFresnelPower;
            uniform float4 _AddFresnel_Color;
            uniform float _AddFresnel_Intensity;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (_FinalMix*(lerp( float2(0,0), lerp( node_7638.rgb.rg, _HasIntersectionDepth_var.rg, _SameDistortionAsOpacity ), _HasDistortion )*_DistortionStrength));
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
////// Lighting:
////// Emissive:
                float4 node_9199 = tex2D(_FinalSS_Add,TRANSFORM_TEX(i.screenPos.rg, _FinalSS_Add));
                float3 _HasFinalSS_Add_var = lerp( _HasIntersectionDepth_var, ((node_9199.rgb*_FinalSS_Add_Power)+_HasIntersectionDepth_var), _HasFinalSS_Add );
                float3 emissive = (lerp( _HasFinalSS_Add_var, ((_HasFinalSS_Add_var+(saturate(pow(1.0-max(0,dot(normalDirection, viewDirection)),_AddFresnelPower))*_AddFresnel_Color.rgb))*_AddFresnel_Intensity), _HasAdditiveFresnel )*_FinalMix);
                ...
                return fixed4(lerp(sceneColor.rgb, finalColor,(_FinalMix*(_FinalOpacity*lerp( _HasOpacityDepth_var, (_HasOpacityDepth_var*node_8690), _HasOpacityFresnel )))),1);
            }
            ENDCG
        }
    }
